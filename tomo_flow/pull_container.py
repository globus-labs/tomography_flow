    ...
    if container_url.startswith(("http", "https")):
        try:
            ...
            if response.status_code != 200:
                return {
                    "error": f"Invalid container URL: {container_url} (HTTP {response.status_code})"
                }
    ...
    try:
        if runtime in ["docker", "podman"]:
            ...
            if local_image:
                ...
                pull_result = subprocess.run(pull_cmd, capture_output=True, text=True)
                return {
                    "status": "Container pulled successfully.",
                    "stdout": pull_result.stdout,
                    "stderr": pull_result.stderr,
                }
            ...
            local_exists = (
                subprocess.run(check_cmd, capture_output=True, text=True).returncode
                == 0
            )
            ...
            if local_exists:
                ...
                pull_result = subprocess.run(pull_cmd, capture_output=True, text=True)
                return {
                    "status": "Updated OCI container to latest version.",
                    "stdout": pull_result.stdout,
                    "stderr": pull_result.stderr,
                }
            else:
                ...
                pull_result = subprocess.run(pull_cmd, capture_output=True, text=True)
                return {
                    "status": "OCI container pulled successfully.",
                    "stdout": pull_result.stdout,
                    "stderr": pull_result.stderr,
                }
